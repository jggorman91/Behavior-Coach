document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DATA: The full, corrected list of built-in strategies ---
    let strategies = {
        'Calling Out': {
            Attention: [
    ...
    // This is a clean copy of the original, built-in strategies for later use
    const builtInStrategies = JSON.parse(JSON.stringify(strategies));
    ...
    function loadAllPlansIntoApp() {
        let customPlans = JSON.parse(localStorage.getItem('customBIPs')) || {};
        
        // Start with a fresh copy of built-in strategies and merge custom ones
        strategies = { ...builtInStrategies, ...customPlans };

        document.querySelectorAll('.custom-bip-option').forEach(opt => opt.remove());

        for (const behavior in customPlans) {
            // Check if an option for this behavior already exists
            if (!commonBehaviorsSelect.querySelector(`option[value="${behavior}"]`)) {
    ...
    function displayNewStrategy() {
        ...
        if (newStrategies.length > 0) {
            ...
            ineffectiveBtn.style.display = 'none';
        }
    }
    ...
    loadPlanBtn.addEventListener('click', () => showScreen('custom-plan-screen'));
    ...
    functionBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (selectedBehavior === 'default') {
                alert('Please select a behavior first.');
                return;
            }

            currentState.attemptedStrategies = [];
            currentState.behavior = selectedBehavior;
            currentState.function = btn.dataset.function;

            ineffectiveBtn.style.display = 'inline-block';
            displayNewStrategy();
